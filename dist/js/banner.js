/*
    ...
    * */
    ...
    var spans=document.querySelectorAll('#bnbtn span')
    var index=1 //按钮的初始下标
    var dsq;//定时器
    ...
    function animation1(offset){
        //当点击当前显示图片对应按钮时，直接跳出函数 当移动距离为0的时候代表不移动，因此就不在执行下面的代码
        if(offset==0){
            return
        }
        //设置每一步的步长
        var speed=offset/40
        //获取图片移动终点
        var left=parseInt(bnimgs.style.left)+offset
        aa=true
        //动起来
        var go= function () {
            //从左往右移动 如果步长是正数且                       从右往左移动
